...
class OptimizedJsonGrammar : Grammar<Any?>() {
  ...
  private val string: Parser<String> by
      (stringLiteral use { input.substring(offset + 1, offset + length - 1) })
  ...
  private val jsonObject: Parser<Map<String, Any?>> by
      (-openingBrace * separated(string * -colon * this, comma, true) * -closingBrace).map {
        mutableMapOf<String, Any?>().apply { it.terms.forEach { put(it.t1, it.t2) } }
      }

  private val jsonArray: Parser<List<Any?>> by
      (-openingBracket * separated(this, comma, true) * -closingBracket).map { it.terms }
