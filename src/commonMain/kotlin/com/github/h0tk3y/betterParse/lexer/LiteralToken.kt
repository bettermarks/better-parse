...
public class LiteralToken(name: String?, public val text: String, ignored: Boolean = false) :
    Token(name, ignored) {
...
public class CharToken(name: String?, public val text: Char, ignored: Boolean = false) :
    Token(name, ignored) {
  override fun match(input: CharSequence, fromIndex: Int): Int =
...
public fun literalToken(name: String, text: String, ignore: Boolean = false): Token {
  if (text.length == 1) return CharToken(name, text[0], ignore)
  return LiteralToken(name, text, ignore)
}

public fun literalToken(text: String, ignore: Boolean = false): Token {
  if (text.length == 1) return CharToken(null, text[0], ignore)
