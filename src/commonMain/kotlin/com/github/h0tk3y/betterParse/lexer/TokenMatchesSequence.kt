...
/**
 * Stateful producer of tokens that yields [Token]s from some inputs sequence that it is based upon,
 * one by one
 */
...
public class TokenMatchesSequence(
  ...
  public fun getNotIgnored(position: Int): TokenMatch? {
    ...
    while (true) {
      val value =
          if (pos < matches.size) matches[pos]
          else {
      ...
      if (!value.type.ignored) return value
      pos++
    }
  }
