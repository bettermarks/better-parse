...
/**
 * A common interface for parsers that can try to consume a part or the whole [TokenMatch] sequence
 * and return one of possible [ParseResult], either [Parsed] or [ErrorResult]
 */
...
public fun <T> Parser<T>.tryParseToEnd(
...
public fun <T> Parser<T>.parse(tokens: TokenMatchesSequence): T =
    tryParse(tokens, 0).toParsedOrThrow().value

public fun <T> Parser<T>.parseToEnd(tokens: TokenMatchesSequence): T =
    tryParseToEnd(tokens, 0).toParsedOrThrow().value

/** Represents a result of input sequence parsing by a [Parser] that tried to parse [T]. */
public sealed class ParseResult<out T>

/**
 * Represents a successful parsing result of a [Parser] that produced [value] and left input
 * starting with [nextPosition] unprocessed.
 */
...
internal class ParsedValue<out T>(override val value: T, override val nextPosition: Int) :
    Parsed<T>()
...
/**
 * A lexer could not match the input sequence with any token known to it. Contains [tokenMismatch]
 * with special type
 */
...
/**
 * Thrown when a [Parser] is forced to parse a sequence with [parseToEnd] or [parse] and fails with
 * an [ErrorResult].
 */
...
/**
 * Throws [ParseException] if the receiver [ParseResult] is a [ErrorResult]. Returns the [Parsed]
 * result otherwise.
 */
public fun <T> ParseResult<T>.toParsedOrThrow(): Parsed<T> =
    when (this) {
      is Parsed -> this
