...
/**
 * Parses the sequence with the receiver [Parser] and then with the [other] parser. If both succeed,
 * returns a [Tuple2] with the values from the [Parsed] results. Otherwise, returns the
 * [ErrorResult] of the failed parser.
 */
public inline infix fun <reified A, reified B> Parser<A>.and(
    other: Parser<B>
): AndCombinator<Tuple2<A, B>> =
    AndCombinator(listOf(this, other)) { (a1, a2) -> Tuple2(a1 as A, a2 as B) }

/** The same as `this `[and]` other` */
public inline operator fun <reified A, reified B> Parser<A>.times(
    other: Parser<B>
): AndCombinator<Tuple2<A, B>> = this and other
 ...
 */
@JvmName("and0")
public inline infix fun <reified A, reified B> AndCombinator<A>.and(
    other: Parser<B>
): AndCombinator<Tuple2<A, B>> =
    AndCombinator(consumersImpl + listOf(other)) { (a1, a2) -> Tuple2(a1 as A, a2 as B) }

/** The same as `this `[and]` other` */
public inline operator fun <reified A, reified B> AndCombinator<A>.times(
    other: Parser<B>
): AndCombinator<Tuple2<A, B>> = this and other

public class AndCombinator<out R>
@PublishedApi
