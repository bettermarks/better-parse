...
public class SeparatedCombinator<T, S>(
  ...
  override fun tryParse(
    ...
    return when (first) {
      is ErrorResult ->
          if (acceptZero) ParsedValue(Separated(emptyList(), emptyList()), fromPosition) else first
...
/**
 * A list of [terms] separated by [separators], which is either empty (both `terms` and
 * `separators`) or contains one more term than there are separators.
 */
public class Separated<T, S>(public val terms: List<T>, public val separators: List<S>) {
  init {
    require(terms.size == separators.size + 1 || terms.isEmpty() && separators.isEmpty())
  }

  /**
   * Returns the result of reducing [terms] and [separators] with [function], starting from the left
   * and processing current result (initially, `terms[0]`), `separators[i]` and `terms[i + 1]` at
   * each step for `i` in `0 until terms.size`.
   *
  ...
  public fun reduce(function: (T, S, T) -> T): T {
    ...
    for (i in separators.indices) result = function(result, separators[i], terms[i + 1])
    return result
  }

  /**
   * Returns the result of reducing [terms] and [separators] with [function], starting from the
   * right and processing `terms[i]`, `separators[i]` and current result (initially, `terms.last()`)
   * at each step for `i` in `terms.size - 1 downTo 0`.
   *
   * @throws [NoSuchElementException] if [terms] are empty
   */
  public fun reduceRight(function: (T, S, T) -> T): T {
    ...
    for (i in separators.indices.reversed()) result = function(terms[i], separators[i], result)
    return result
  }
}

/**
 * Parses a chain of [term]s separated by [separator], also accepting no matches at all if
 * [acceptZero] is true.
 */
...
/**
 * Parses a chain of [term]s separated by [separator], also accepting no matches at all if
 * [acceptZero] is true, and returning only matches of [term].
 */
...
/**
 * Parses a chain of [term]s separated by [operator] and reduces the result with
 * [Separated.reduceRight].
 */
public inline fun <reified T, reified S> rightAssociative(
    term: Parser<T>,
    operator: Parser<S>,
    noinline transform: (T, S, T) -> T
): Parser<T> = separated(term, operator) map { it.reduceRight(transform) }
