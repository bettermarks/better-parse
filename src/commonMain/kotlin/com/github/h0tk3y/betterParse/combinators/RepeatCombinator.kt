...
/**
 * Tries to parse the sequence with the [parser], as many times as it succeeds, but no more than
 * [atMost]. If the parser succeeded less than [atLeast] times, returns its [ErrorResult], otherwise
 * returns the list of [Parsed] results from the parser invocations.
 */
public class RepeatCombinator<T>
internal constructor(
  ...
  init {
    ...
    require(atMost == -1 || atMost >= atLeast) {
      "atMost = $atMost is invalid, should be greater or equal than atLeast = $atLeast"
    }
  }
  ...
  override fun tryParse(tokens: TokenMatchesSequence, fromPosition: Int): ParseResult<List<T>> {
    ...
    while (atMost == -1 || resultsList.size < atMost) {
      ...
      when (result) {
        is ErrorResult -> {
          return if (resultsList.size >= atLeast) ParsedValue(resultsList, nextPosition) else result
        }
        is Parsed -> {
...
public infix fun <T> Int.timesOrMore(parser: Parser<T>): Parser<List<T>> =
    RepeatCombinator(parser, atLeast = this)
