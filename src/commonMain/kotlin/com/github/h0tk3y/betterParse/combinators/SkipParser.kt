...
/**
 * Parses the sequence with the receiver [Parser] and the wrapped [other] parser, but returns the
 * [Parsed] result from the receiver parser.
 */
public infix fun <T : Tuple> AndCombinator<T>.and(other: SkipParser): AndCombinator<T> =
    AndCombinator(consumersImpl + other, transform)

public operator fun <T : Tuple> AndCombinator<T>.times(other: SkipParser): AndCombinator<T> =
    this and other

/**
 * Parses the sequence with the receiver [Parser] and the wrapped [other] parser, but returns the
 * [Parsed] result with a value from the receiver parser in a [Tuple1].
 */
...
public inline operator fun <reified T> Parser<T>.times(other: SkipParser): AndCombinator<T> =
    this and other

/**
 * Parses the wrapped receiver [Parser] and the [other] parser and returns the [Parsed] result with
 * a value from the [other] parser in a [Tuple1].
 */
public inline infix fun <reified T> SkipParser.and(other: Parser<T>): AndCombinator<T> =
    AndCombinator(listOf(this, other)) { (b) -> b as T }

/** The same as `this `[and]` other` */
public inline operator fun <reified T> SkipParser.times(other: Parser<T>): AndCombinator<T> =
    this and other

public infix fun SkipParser.and(other: SkipParser): SkipParser {
  val parsersLeft =
      if (innerParser is AndCombinator) innerParser.consumersImpl else listOf(innerParser)
