...
/**
 * Encloses custom logic for transforming a [Parser] to a parser of [SyntaxTree]. A correct
 * implementation overrides [liftToSyntaxTree] so that it calls `recurse` for the sub-parsers, if
 * any, and combines them into the parser that it returns.
 */
public interface LiftToSyntaxTreeTransformer {
  ...
  public fun <T> liftToSyntaxTree(
      parser: Parser<T>,
      default: DefaultTransformerReference
  ): Parser<SyntaxTree<T>>
}
 ...
 *
 * @param retainSkipped - whether the [skip]ped parsers should be present in the syntax tree
 *   structure.
 * @param retainSeparators - whether the separators of [separated], [leftAssociative] and
 *   [rightAssociative] should be present in the syntax tree structure.
 */
...
/**
 * Converts a [Parser] of [T] to another that parses [SyntaxTree]. The resulting [SyntaxTree]s will
 * have this parser as [SyntaxTree.parser] and the result that this parser is stored in
 * [SyntaxTree.item]. The [liftOptions] are used to determine which parts of the syntax tree should
 * be dropped. The [structureParsers] define the resulting structure of the syntax tree: only the
 * nodes having these parsers are retained (see: [SyntaxTree.flatten]), pass empty set to retain all
 * nodes.
 */
public fun <T> Parser<T>.liftToSyntaxTreeParser(
  ...
  return if (structureParsers == null) astParser else astParser.flattened(structureParsers)
}

/**
 * Converts a [Grammar] so that its [Grammar.rootParser] parses a [SyntaxTree]. See:
...
public fun <T> Grammar<T>.liftToSyntaxTreeGrammar(
    ...
    transformer: LiftToSyntaxTreeTransformer? = null
): Grammar<SyntaxTree<T>> =
    object : Grammar<SyntaxTree<T>>() {
      override val rootParser: Parser<SyntaxTree<T>> =
          this@liftToSyntaxTreeGrammar.rootParser.liftToSyntaxTreeParser(
              liftOptions, structureParsers, transformer)

      override val tokens: List<Token>
        get() = this@liftToSyntaxTreeGrammar.tokens
...
private class ParserToSyntaxTreeLifter(
  ...
  fun <T> lift(parser: Parser<T>): Parser<SyntaxTree<T>> {
    if (parser in parsersInStack) return referenceResultInStack(parser) as Parser<SyntaxTree<T>>
    ...
    val result =
        when (parser) {
          ...
          is SeparatedCombinator<*, *> -> liftSeparatedCombinatorToAST(parser)
            ...
            transformer?.liftToSyntaxTree(parser, default)
                ?: throw IllegalArgumentException(
                    "Unexpected parser $this. Provide a custom transformer that can lift it.")
          }
        }
  ...
  inner class DefaultTransformerReference :
      LiftToSyntaxTreeTransformer.DefaultTransformerReference {
  ...
  private fun <T, R> liftMapCombinatorToAST(
      combinator: MapCombinator<T, R>
  ): Parser<SyntaxTree<R>> {
  ...
  private fun <T> liftOptionalCombinatorToAST(
      combinator: OptionalCombinator<T>
  ): Parser<SyntaxTree<T?>> {
  ...
  private fun <T> liftAndCombinatorToAST(
      combinator: AndCombinator<T>
  ): AndCombinator<SyntaxTree<T>> {
    val liftedConsumers =
        combinator.consumersImpl.map {
          when (it) {
            is Parser<*> -> lift(it)
    ...
    return AndCombinator(liftedConsumers) { parsedItems ->
      ...
      val originalResult =
          combinator.transform(nonSkippedResults.map { (it as SyntaxTree<*>).item })
      val start = (parsedItems.first() as SyntaxTree<*>).range.first
      val end =
          ((parsedItems.lastOrNull { (it as SyntaxTree<*>).range.last != 0 }) as? SyntaxTree<*>)
              ?.range
              ?.last ?: 0
      @Suppress("UNCHECKED_CAST")
      val children =
          if (liftOptions.retainSkipped) parsedItems as List<SyntaxTree<*>>
          else combinator.nonSkippedIndices.map { parsedItems[it] } as List<SyntaxTree<*>>
  ...
  private fun <T> liftRepeatCombinatorToAST(
      combinator: RepeatCombinator<T>
  ): Parser<SyntaxTree<List<T>>> {
  ...
  private fun <T, S> liftSeparatedCombinatorToAST(
      combinator: SeparatedCombinator<T, S>
  ): Parser<SyntaxTree<Separated<T, S>>> {
    ...
    return SeparatedCombinator(liftedTerm, liftedSeparator, combinator.acceptZero).map { separated
      ...
      val children =
          when {
            ...
            liftOptions.retainSeparators ->
                listOf(separated.terms.first()) +
                    (separated.separators zip separated.terms.drop(1)).flatMap { (s, t) ->
                      listOf(s, t)
                    }
  ...
  private fun emptyASTParser() =
      EmptyParser.map { SyntaxTree(Unit, emptyList(), EmptyParser, 0..0) }
