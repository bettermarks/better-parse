...
/**
 * A language grammar represented by a list of [Token]s and one or more [Parser]s, with one specific
 * [rootParser] that accepts the words of this [Grammar].
 */
public abstract class Grammar<out T> : Parser<T> {
   ...
   * The tokens are added to this list during an instance construction.
   */
  public open val tokens: List<Token>
    get(): List<Token> = _tokens.distinctBy { it.name ?: it }

  /**
   * Set of the tokens and parsers that were declared by delegation to the parser instances (`val p
   * by someParser`), and [rootParser]
   */
  public open val declaredParsers: Set<Parser<*>>
    get() = (_parsers + _tokens + rootParser).toSet()

  /**
   * A [Tokenizer] that is built with the [Token]s defined within this [Grammar], in their order of
   * declaration
   */
  ...
  final override fun tryParse(tokens: TokenMatchesSequence, fromPosition: Int): ParseResult<T> =
      rootParser.tryParse(tokens, fromPosition)

  protected operator fun <T> Parser<T>.provideDelegate(
      thisRef: Grammar<*>,
      property: KProperty<*>
  ): Parser<T> = also { _parsers.add(it) }
...
public class ParserReference<out T> internal constructor(parserProvider: () -> Parser<T>) :
    Parser<T> {
