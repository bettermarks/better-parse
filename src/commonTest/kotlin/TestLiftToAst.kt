...
private data class And(val left: BooleanExpression, val right: BooleanExpression) :
    BooleanExpression()

private data class Or(val left: BooleanExpression, val right: BooleanExpression) :
    BooleanExpression()

private data class Impl(val left: BooleanExpression, val right: BooleanExpression) :
    BooleanExpression()

private val booleanGrammar =
    object : Grammar<BooleanExpression>() {
...
private fun SyntaxTree<*>.toTopDownStrings() =
    topDownNodesSequence()
        .filter { (it.children.isEmpty() || it.children.size > 1) }
        .map {
          val item = it.item
          if (item is TokenMatch) item.text else it.item?.typeName()
        }
        .toList()
...
internal class TestLiftToAst {
  ...
  fun continuousRange() {
    val astParser =
        booleanGrammar.liftToSyntaxTreeGrammar(
            LiftToSyntaxTreeOptions(retainSkipped = true), structureParsers = null)
  ...
  fun testDropSkipped() {
    val astParser =
        booleanGrammar.liftToSyntaxTreeGrammar(
            LiftToSyntaxTreeOptions(retainSkipped = false), structureParsers = null)
    val ast = astParser.parseToEnd("(!(a)->((b)|(!c))->!(!a)&!(d))")
    val types = ast.toTopDownStrings()

    val expected =
        listOf("Separated", "a", "->", "Separated", "b", "|", "c", "->", "Separated", "a", "&", "d")
    assertEquals(expected, types)
  }
  ...
  fun testFlattening() {
    ...
    val booleanAstGrammar =
        booleanGrammar.liftToSyntaxTreeGrammar(LiftToSyntaxTreeOptions(retainSeparators = false))
  ...
  fun testDropSeparators() {
    ...
    val booleanAstGrammar =
        booleanGrammar.liftToSyntaxTreeGrammar(
            LiftToSyntaxTreeOptions(retainSkipped = false, retainSeparators = false))
    val ast = booleanAstGrammar.parseToEnd(expr)
  ...
  fun testCustomTransformer() {
      ...
      override fun tryParse(
        ...
        val res =
            alternatives
                ...
                .firstOrNull { it.second is Parsed<T> } ?: return object : ErrorResult() {}
        val (parser1, res1) = res
        val res2 = parser1.tryParse(tokens, res1.toParsedOrThrow().nextPosition)
        return when (res2) {
          is ErrorResult -> res2
          is Parsed<T> -> ParsedValue(res1.toParsedOrThrow().value to res2.value, res2.nextPosition)
        }
      }
    }

    val transformer =
        object : LiftToSyntaxTreeTransformer {
    ...
    val lifted =
        parser.liftToSyntaxTreeParser(
            structureParsers = booleanGrammar.declaredParsers, transformer = transformer)

    val result = lifted.tryParse(booleanGrammar.tokenizer.tokenize("||"), 0)
    val value = result.toParsedOrThrow().value

    @Suppress("USELESS_IS_CHECK") assertTrue(value is SyntaxTree<*>)
