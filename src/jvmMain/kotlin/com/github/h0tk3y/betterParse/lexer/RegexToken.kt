...
public actual class RegexToken
private constructor(
    name: String?,
    ignored: Boolean,
  ...
  private companion object {
    ...
    private fun prependPatternWithInputStart(patternString: String, options: Set<RegexOption>) =
        if (patternString.startsWith(Companion.inputStartPrefix)) patternString.toRegex(options)
        else {
          ...
          ("${inputStartPrefix}(?:$patternToEmbed$newlineAfterComments)").toRegex(
              options - RegexOption.LITERAL)
        }
  }
  ...
  public actual constructor(
      ...
      ignored: Boolean
  ) : this(name, ignored, patternString, prependPatternWithInputStart(patternString, emptySet()))

  public actual constructor(
      name: String?,
      regex: Regex,
      ignored: Boolean
  ) : this(name, ignored, regex.pattern, prependPatternWithInputStart(regex.pattern, regex.options))
