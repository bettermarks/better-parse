...
fun main() {
  ...
  (exprs.asSequence() + readExprSequence).forEach {
    parseAndPrintTree(it)
    println("\n")
  }
}
...
fun printSyntaxTree(expr: String, syntaxTree: SyntaxTree<*>) {
  ...
  while (currentLayer.isNotEmpty()) {
    val underscores =
        currentLayer.flatMap { t -> t.range.map { index -> index to charByTree(t) } }.toMap()
...
fun charByTree(tree: SyntaxTree<*>): Char =
    with(BooleanGrammar) {
      when (tree.parser) {
        id -> 'i'
        and,
